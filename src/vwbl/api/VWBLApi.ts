...
export class VWBLApi {
  ...
  async setKey(documentId: string, chainId: number, key: string, signature: string, hasNonce?: boolean, autoMigration?: boolean) {
    await this.instance.post("/keys", { document_id: documentId, chain_id: chainId, key, signature, has_nonce: hasNonce, auto_migration: autoMigration });
  }
