...
export class VWBLMetaTx extends VWBLBase {
   ...
   */
  mintToken: MintTokenMetaTx = async (
    feeNumerator: number,
    mintApiId: string
  ): Promise<number> => {
   ...
   */
  revokeViewPermission = async (
    tokenId: number,
    revoker: string,
    revokeViewPermisionApiId: string
  ): Promise<void> => {
    await this.nft.revokeViewPermission(
      tokenId,
      revoker,
      revokeViewPermisionApiId
    );
  };
