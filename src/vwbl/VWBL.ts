...
export class VWBL {
   ...
   * @returns Encrypted file data
   */
  encryptStream = async (plainFile: File, key: string): Promise<stream> => {
   ...
   * @returns Encrypted file data
   */
  decryptStream = async (encryptFile: stream, key: string): Promise<stream> => {
      ...
      encryptedDataUrls.map(async (encryptedDataUrl) => {
        const encryptedData = (
          await axios.get(encryptedDataUrl, {
            responseType:
              encryptLogic === "base64" ? "text" : isRunningOnBrowser ? "arraybuffer" : "stream",
          })
        ).data;
